...
async function mostrarPantallaInicio() {
  ...
  try {
    ...
    contenedorPrincipal.appendChild(crearSeccion('Top 10 en Tendencia',  tendencias.slice(0, 10), 'top'));
    contenedorPrincipal.appendChild(crearSeccion('Películas Populares',  peliculasPopulares));
    contenedorPrincipal.appendChild(crearSeccion('Series Populares',     seriesPopulares));
    contenedorPrincipal.appendChild(crearSeccion('Las Mejor Valoradas',  peliculasMejorValoradas));
    contenedorPrincipal.appendChild(crearSeccion('Ahora en Cines',       enCartelera));

    /* Sidebar con los más populares de todas las categorías */
    renderizarSidebar([...tendencias, ...peliculasPopulares, ...seriesPopulares]);
...
async function mostrarPantallaPeliculas() {
  ...
  try {
    ...
    contenedorPrincipal.innerHTML = '';
    contenedorPrincipal.appendChild(crearSeccion('Películas Populares', populares));
    contenedorPrincipal.appendChild(crearSeccion('Mejor Valoradas',     mejorValoradas));
    contenedorPrincipal.appendChild(crearSeccion('Ahora en Cines (MX)', enCartelera));
  } catch (error) {
...
async function mostrarPantallaSeries() {
  ...
  try {
    ...
    contenedorPrincipal.innerHTML = '';
    contenedorPrincipal.appendChild(crearSeccion('Series Populares',       populares));
    contenedorPrincipal.appendChild(crearSeccion('Series Mejor Valoradas', mejorValoradas));
  } catch (error) {
...
async function mostrarPantallaDeGenero(genero) {
  ...
  try {
    ...
    if (peliculas.length) {
      contenedorPrincipal.appendChild(crearSeccion(`${nombreGenero} — Películas`, peliculas));
    }
    if (series.length) {
      contenedorPrincipal.appendChild(crearSeccion(`${nombreGenero} — Series`, series));
    }
    ...
    mostrarPantallaDeError(contenedorPrincipal, () => mostrarPantallaDeGenero(genero));
  }
}
...
(async function iniciarAplicacion() {
  ...
  try {
    ...
    listaDeGeneros = [...generosDePeliculas, ...generosDeSeries].filter(genero => {
  ...
  document.getElementById('navPeliculas')?.addEventListener('click', evento => {
    evento.preventDefault();
